...
- Email-gated writes: `submit-question`, `vote-question`, `submit-video-answer`, and the `verify-candidate-claim` initiate action reject anonymous callers with `403 EMAIL_REQUIRED`. Client wraps writes in `EmailGateProvider`; failure opens `EmailGateModal` (magic link) and persists a `PendingIntent` to localStorage. `usePendingIntentRunner` (mounted at the `/app` shell) replays the intent after `AUTH_UPGRADED` and clears it. `questions.asked_by` FK ties authored content to the verified user (ON DELETE SET NULL).
