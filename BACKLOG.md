...
| B6-1 | Phase 1: `verify-candidate-claim` Edge Function (federal path) | in-progress | Schema landed (1a): `pending_claims` (UNIQUE partial index on candidate_id WHERE status='pending'), `candidate_registry` (verification-only), `audit_pending_claim` trigger, `expire_stale_pending_claims` 5-min pg_cron, `revoke_candidate_claim(uuid, text)` RPC for sybil revocation. Remaining: 1b verify-candidate-claim Edge Function (FEC API, magic-link initiate/finalize), 1c multi-step claim modal UI + sybil revoke action in /admin/dedup. Retires the existing `claim-candidate`. |
