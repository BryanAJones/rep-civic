...
| B6-1 | Phase 1: `verify-candidate-claim` Edge Function (federal path) | in-progress | Schema landed (1a): pending_claims, candidate_registry, audit_pending_claim trigger, 5-min pending-claims-expire pg_cron, revoke_candidate_claim RPC. Edge Function landed (1b): `verify-candidate-claim` with action='initiate' (federal path: cache-first FEC API lookup via _shared/fec.ts → committee email → magic link via signInWithOtp → pending_claims row, OR no email → social_proof_required code) and action='finalize' (resolves pending_claims by user.email → promote → candidate_claims insert → status flip). 10/hr per-user rate limit. State/local return 501. Remaining: 1c multi-step claim modal UI, /app/claim/finalize route, sybil revoke action in /admin/dedup. Retires `claim-candidate` in 1c when UI fully migrates. |
