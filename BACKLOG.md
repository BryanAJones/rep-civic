...
| B1-20 | State-level challenger data source (scraper) | standby | Demoted from planned to standby 2026-04-17. Ballotpedia per-race scraping remains the fallback path, not the primary. Only build if B1-24 concludes with "no outreach track yielded a usable source." Bot identification page (B1-21) and User-Agent string are prerequisites already in place. Bulk paid CSV ~$500-600 one-time (data@ballotpedia.org) is a pre-scraper alternative if we change our mind on budget. |
...
| B1-22 | Google Civic voterInfoQuery integration | in-progress | Hybrid election-window layer on top of baseline. `proxy-voterinfo` Edge Function + `getBallotForAddress` service method + onboarding wire-up (commits 1-3) done and deployed. Remaining: admin dedup UI + pg_cron cache cleanup + observability (commit 4), FEC retirement (commit 5, gated on post-primary 2026-05-19). Live today but returns `{source:'fallback'}` because Google has not yet loaded GA 2026 primary data; expected to activate ~2-3 weeks before election day. Confirmed seasonal (~4-5 months/year of live coverage), which motivated B1-23. |
| B1-23 | Year-round candidate source outreach | planned | Parallel outreach to four tracks: Civic Forge Solutions (Atlanta-based, covers all 2026 GA qualified candidates incl. local; API terms unknown), Vote Smart (public API, verify GA coverage), Democracy Works Elections API (request non-profit/pre-revenue tier), Wikidata SPARQL (free incumbent baseline cross-check). Closes when each track has a definitive yes/no/price response. |
| B1-24 | Year-round source selection decision | planned | Blocked by B1-23. Pick the primary year-round source based on coverage (challengers + local statewide), access model, freshness, and terms. Record one-sentence rationale in CHANGELOG.md. |
| B1-25 | Integrate chosen year-round source | planned | Blocked by B1-24. Pattern identical to existing source imports (`scripts/import/sources/<source>.ts` + merge into `transform.ts` with `sources=['<source_name>']` tag). Nightly refresh via existing GitHub Action. Fallback to B1-20 scraper retained on standby. |
