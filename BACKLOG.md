...
| B1-22 | Google Civic voterInfoQuery integration | in-progress | Hybrid election-window layer on top of baseline. `proxy-voterinfo` Edge Function + `getBallotForAddress` service method + onboarding wire-up (commits 1-3) done. Remaining: admin dedup UI + pg_cron cache cleanup + observability (commit 4), FEC retirement (commit 5, gated on post-primary 2026-05-19). During election windows, ballot includes US Senate/House, state lege, county, city, judicial, school board. Closes B1-20 challenger gap during election windows and adds full local-race coverage. Requires `GOOGLE_CIVIC_API_KEY` secret. |
