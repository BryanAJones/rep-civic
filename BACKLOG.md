...
| B6-11 | Fix seed-test-claim idempotency | planned | `scripts/seed-test-claim.ts` `upsert` with `onConflict: 'filing_id'` is creating new candidate IDs each run instead of preserving them, which breaks the test URL between runs. Confirmed 2026-05-12. Either lock onto a fixed UUID or switch to update-or-insert keyed on `filing_id`. Low priority — only affects internal dogfooding. |
| B6-12 | Diagnose mobile claim "non-2xx" repro | idea | User hit Edge Function non-2xx on a single click during dogfooding 2026-05-12. Probe with a fresh user returned 200 cleanly; pending_claims and candidate_claims for the user were empty; verified user account exists and is_anonymous=false. Suspected stale browser session JWT but never confirmed (couldn't open DevTools on the device). If reproduces with a new claimant, add client-side error logging to capture status + body, then triage. |
