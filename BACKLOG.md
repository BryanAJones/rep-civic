...
| 52 | Full onboarding flow (multi-step) | done | Skeleton loading + ballot page + share shipped. Map thumbnails deferred 2026-05-12 — visual polish with no learning value. Onboarding works; users get to their ballot. Re-evaluate if real-user feedback specifically calls out the missing spatial context. |
...
| 56 | Workbox caching strategies | deferred | **Deferred 2026-05-12.** Workbox defaults work. Custom strategies are PWA polish that matters when there's real traffic to protect — not yet. Re-evaluate when usage data shows specific caching gaps. |
| 57 | Offline behavior + fallback | deferred | **Deferred 2026-05-12.** Same reason as 56 — offline experience matters with installed users who lose connectivity mid-session. Until installed-user base exists, offline fallback is a vanity ship. Re-evaluate after the product has installed users. |
...
| B1-18 | Nightly candidate data refresh (GitHub Action) | done | Shipped 2026-04-09 in commit `d179bbc` (initial workflow) + `2fbe6f9` (inline Supabase URL fix). `.github/workflows/refresh-candidates.yml` runs `npm run import:all` daily at 07:00 UTC (03:00 ET) with `workflow_dispatch` for manual triggers. Repo secrets: `CONGRESS_API_KEY`, `SUPABASE_SERVICE_KEY`. Verified 2026-05-12: 5 consecutive successful runs (May 8–12), ~25–35s each. Backlog status was stale — work was live for a month before being marked done. |
...
| B6-1 | Phase 1: `verify-candidate-claim` Edge Function (federal path) | done | Shipped 2026-05-05 in 0.18.0. Schema (pending_claims with UNIQUE partial index, candidate_registry, audit_pending_claim trigger, expire_stale_pending_claims pg_cron, revoke_candidate_claim RPC) + Edge Function (initiate/finalize dispatch, FEC API client with 24h cache, 10/hr rate limit, social_proof_required branch stub) + UI (multi-step ClaimModal, /app/claim/finalize route, sybil revoke in /admin/dedup). claim-candidate Edge Function source deleted 2026-05-05 (was verification bypass); the deployed function was missed at the time and only removed 2026-05-12 via `supabase functions delete claim-candidate`. Audit-log query of all `candidate.claimed` events shows 0 `self_attestation` claims (the bypass signature) and 0 surviving `candidate_claims` rows with that method — the gap was open but unexploited. State/local return 501; phase 4/6 wire those. |
| B6-2 | Phase 2: Empty-state copy + Web Share + dynamic OG image | done | Copy + share landed 2026-05-05. New `ProfileShareButton` (Web Share API + clipboard fallback), calm-dossier rewording across `EmptyVideoGrid` and the profile-page stubs, mono counts via `MonoText`. **OG image piece deferred 2026-05-12** — dynamic Cloudflare Worker `/og?candidate=[id]` matters for share-preview cards when candidates share their claimed profile. Until candidates actually claim and share, OG previews are speculative polish. Re-evaluate after the first claimed candidate uses `ProfileShareButton` in the wild. |
| B6-3 | Phase 3: Ballotpedia verification registry scraper | deferred | **Deferred 2026-05-12 per strategic reassessment.** The real bottleneck on the B6 path is not candidate inventory — it's zero validated claims. Federal coverage already exists via Congress.gov + FEC; state incumbents via OpenStates. Adding the state-challenger off-cycle gap does not move the dial on "will any real candidate claim?" Same logic as B1-23/B1-24: narrow the goal, don't widen the sources. Re-evaluate after SMTP (B6-10) + social-handle fallback (B6-5) ship and at least one real candidate completes a claim — if the bottleneck then becomes inventory, revisit. |
| B6-4 | Phase 4: State verification path + GA SOS Playwright fallback | deferred | **Deferred 2026-05-12.** Cascades from B6-3 — without the Ballotpedia registry to fall through from, the two-tier verifyState has nothing to be tier-1. Re-evaluate alongside B6-3. |
| B6-5 | Phase 5: Social-handle fallback for candidates without on-file email | in-progress | **Phase 5a shipped 2026-05-12.** Migration `20260512000000_b6_5_social_proof_url.sql` adds `social_proof_url` + `social_proof_submitted_at` to `pending_claims` plus `approve_social_proof` / `reject_social_proof` SECURITY DEFINER RPCs (service-role-only). `verify-candidate-claim` Edge Function gets a new `submit_social_proof` action — claimant pastes the URL where they posted the 9-char code, function validates URL + ownership, stamps the row. `ClaimModal` social-proof step now has an input + submit + terminal "Submitted for review" state. Admin path: new `scripts/review-social-proof.ts` (`npm run review:social-proof`) lists pending submissions and runs approve/reject via the RPCs. **Phase 5b remaining (deferred until first real social-proof claim attempt):** automated verification (campaign-domain email magic-link as second-factor, Twitter API scraping if it pencils out), admin UI inside `/admin/dedup` to replace the CLI, structured rejection-reason taxonomy. Manual review is fine for the first 1–10 candidates. |
...
| B6-8 | Local self-claim (county/city/school board) | deferred | **Deferred 2026-05-12 per strategic reassessment.** Same "untested hypothesis" problem as B6-3 — adding the local claim surface only matters if candidates at the federal/state level actually claim first. Local races also need a design pass (bootstrap-claim mode + identity verification without a filing registry) that costs design time better spent on dogfooding existing flows. Re-evaluate after one real federal candidate completes a claim. |
...
| B6-10 | Custom SMTP for magic-link sends | planned | Default Supabase sender caps at ~4 emails/hr project-wide — fine for dev, blocks real claimants. Configure SendGrid/Resend/AWS SES under Auth → SMTP Settings. Code already surfaces `OTP_RATE_LIMIT` distinctly (429 + code) so the modal copy gracefully degrades while waiting. Required before any real candidate self-claim attempt. **Full setup checklist at `docs/SMTP_SETUP.md` (added 2026-05-12) — needs you to pick a provider (Resend recommended), add the API key in Supabase Dashboard → Auth → SMTP Settings, and verify with a real magic-link send.** |
